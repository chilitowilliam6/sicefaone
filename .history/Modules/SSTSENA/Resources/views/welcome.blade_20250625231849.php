...
<header class="fixed top-0 w-full bg-white shadow z-50">
 ...
 <!-- Menú principal -->
<nav id="menu" class="hidden md:flex flex-col md:flex-row md:items-center gap-6 text-base font-semibold text-gray-700 bg-white md:bg-transparent shadow-lg md:shadow-none py-4 px-6 md:p-0 rounded-lg md:rounded-none absolute md:static top-20 right-4 transition-all duration-300 ease-in-out">
    @auth
        @if (checkRol('sstsena.admin'))
            <li class="list-none">
                <a class="relative px-3 py-2 hover:text-orange-600 transition-colors duration-200 ease-in-out hover:after:absolute hover:after:bottom-0 hover:after:left-0 hover:after:w-full hover:after:h-0.5 hover:after:bg-orange-600" href="{{ route('sstsena.admin.welcome') }}">Administrador</a> 
            </li>
        @endif
        @if (checkRol('sstsena.funcionario'))
            <li class="list-none">
                <a class="relative px-3 py-2 hover:text-orange-600 transition-colors duration-200 ease-in-out hover:after:absolute hover:after:bottom-0 hover:after:left-0 hover:after:w-full hover:after:h-0.5 hover:after:bg-orange-600" href="{{ route('sstsena.funcionario.welcome') }}">Funcionario</a> 
            </li>
        @endif
        ...
        <li class="list-none">
            <a href="#" 
               ...
               class="relative px-3 py-2 text-red-600 hover:text-orange-600 transition-colors duration-200 ease-in-out hover:after:absolute hover:after:bottom-0 hover:after:left-0 hover:after:w-full hover:after:h-0.5 hover:after:bg-orange-600">
                Cerrar Sesión
            </a>
    ...
    @guest
        @if (Route::has('login'))
            <a class="relative px-3 py-2 hover:text-orange-600 transition-colors duration-200 ease-in-out hover:after:absolute hover:after:bottom-0 hover:after:left-0 hover:after:w-full hover:after:h-0.5 hover:after:bg-orange-600" href="{{ route('login') }}">{{ __('Login') }}</a>
        @endif
    @endguest
</nav>
...
<main class="relative pt-32 pb-24 bg-gradient-to-br from-white via-gray-100 to-white">
  ...
  <div class="relative max-w-7xl mx-auto px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-16 items-center z-10">
    ...
    @if (checkRol('sstsena.admin'))
        <a href="{{ route('events.index')}}" class="inline-flex items-center gap-2 px-6 py-3 bg-orange-500 text-white rounded-lg font-semibold hover:bg-orange-600 transition">
            <i class="bi bi-bar-chart-line-fill"></i> Ver Reportes
        </a>
    @endif

    @if (checkRol('sstsena.funcionario'))
        <a href="{{ route('events.index')}}" class="inline-flex items-center gap-2 px-6 py-3 bg-orange-500 text-white rounded-lg font-semibold hover:bg-blue-600 transition">
            <i class="bi bi-bar-chart-line-fill"></i> Historial
        </a>
    @endif
    ...
    <div class="relative">
      <div class="rounded-3xl shadow-xl ring-2 ring-orange-200 overflow-hidden transform hover:scale-105 transition duration-500">
        <img src="{{ asset('SSTSENA/images/carrusel1.jpg') }}" alt="Seguridad en el trabajo" class="w-full h-full object-cover" />
      </div>
  ...
  <div class="max-w-7xl mx-auto px-6 lg:px-8 text-center">
   ...
   <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
    <div class="flex flex-col items-center">
        ...
        <h4 class="text-2xl font-semibold text-gray-700">{{ number_format($totalReports, 0, ',', '.') }}</h4>
        <p class="text-gray-600 mt-2">Reportes Totales</p>
    </div>

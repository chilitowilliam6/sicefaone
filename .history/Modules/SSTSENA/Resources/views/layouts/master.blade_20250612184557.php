A continuación, te proporciono el código HTML completo, incluyendo los cambios solicitados para que el botón de las tres rayitas (toggle) haga que la sidebar se desplace parcialmente hacia la izquierda, dejando una pequeña porción visible, y ajuste el contenido principal y el footer para una mejor visualización. Los cambios se realizaron en el CSS y JavaScript, asegurando que no se alteren la lógica, las rutas, ni la funcionalidad del backend. Este código reemplaza el archivo original que proporcionaste, manteniendo toda la estructura y funcionalidad intactas.
    ...
    <div class="sidebar" id="sidebar">
        ...
        <nav class="nav flex-column p-2">
            ...
            <a href="{{ route('events.index')}}" class="nav-link">
                <i class="fas fa-hourglass-half me-2"></i>Respuesta de eventos
            </a>
    ...
    <script>
        window.addEventListener('load', function() {
            ...
            sidebarToggle.addEventListener('click', function() {
                sidebar.classList.toggle('semi-hidden');
                mainContent.classList.toggle('semi-hidden');
                footer.classList.toggle('semi-hidden');
            });
            ...
            document.addEventListener('click', function(e) {
                if (window.innerWidth <= 768 && !e.target.closest('#sidebar') && !e.target.closest('#sidebarToggle')) {
                    ...
                    mainContent.classList.remove('semi-hidden');
                    footer.classList.remove('semi-hidden');
                }
            });
...
### Cambios realizados
1. **CSS**:
   - Se modificó la clase `.sidebar` para usar `transform` en lugar de `all` en la propiedad `transition`, asegurando una animación suave solo para el desplazamiento.
   - Se agregó la clase `.semi-hidden` para desplazar la sidebar 220px a la izquierda (`translateX(-220px)`), dejando 40px visibles.
   - Se ajustaron los márgenes del `.main-content` y `footer` con `.semi-hidden` para que el contenido ocupe más espacio cuando la sidebar está semi-oculta.
   - Se añadió un pseudo-elemento `::before` en `.sidebar.semi-hidden` para mostrar un ícono de Font Awesome (hamburguesa) en el borde derecho, permitiendo reabrir la sidebar.
   - En la media query para móviles (`max-inline-size: 768px`), se mantuvo el comportamiento original de la sidebar (oculta con `margin-inline-start: -260px`, visible con `.show`) y se soporta `.semi-hidden` sin afectar los márgenes del contenido principal ni del footer.

2. **JavaScript**:
   - Se reemplazó el toggle de la clase `show` por `semi-hidden` en el evento de clic del `#sidebarToggle`, aplicándolo a la sidebar, el contenido principal y el footer.
   - Se mantuvo la lógica para cerrar la sidebar en móviles al hacer clic fuera, removiendo la clase `semi-hidden`.
   - El resto del código (preloader, resaltar enlaces activos) se preservó sin cambios.

3. **Preservación de la lógica**:
   - No se modificaron las directivas de Blade (`@auth`, `@if`, `@yield`, etc.), las rutas (`{{ route(...) }}`), ni la estructura HTML, asegurando que el backend, la autenticación, y los roles funcionen sin problemas.
   - Los cambios son exclusivamente de frontend, por lo que no afectan la funcionalidad de Laravel ni las rutas definidas.

### Funcionalidad
- Al hacer clic en el botón de las tres rayitas (`#sidebarToggle`), la sidebar se desplaza 220px a la izquierda, dejando 40px visibles con un ícono de hamburguesa para reabrirla.
- El contenido principal y el footer se ajustan para ocupar más espacio, mejorando la visualización.
- En móviles, la sidebar sigue oculta por defecto y aparece completamente con `.show`, pero también soporta el estado semi-oculto.
- Al hacer clic fuera de la sidebar en móviles, se cierra (remueve `.semi-hidden`).
- Las rutas, menús desplegables (accordions), notificaciones, y el logout permanecen intactos.

### Personalización
- Puedes ajustar `translateX(-220px)` y `margin-left: 40px` en las clases `.semi-hidden` para cambiar cuánto de la sidebar queda visible.
- Cambia el ícono en `.sidebar.semi-hidden::before` modificando `content: '\f0c9'` por otro código de Font Awesome (ej. `\f053` para una flecha).
- Verifica que los recursos externos (Bootstrap, Font Awesome, jQuery, SweetAlert2) estén accesibles.

Este código es completo y reemplaza el original. Si necesitas algo más o encuentras algún problema, ¡dímelo!
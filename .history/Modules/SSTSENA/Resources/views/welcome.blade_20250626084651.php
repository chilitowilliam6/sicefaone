    ...
    <nav id="menu" class="hidden md:flex flex-col md:flex-row md:items-center gap-4 md:gap-6 text-sm font-medium text-gray-600 absolute md:static top-20 right-4 bg-white md:bg-transparent shadow md:shadow-none p-4 md:p-0 rounded-md md:rounded-none">
      @auth
        @if (checkRol('sstsena.admin'))
          <li><a class="hover:text-orange-500 transition" href="{{ route('sstsena.admin.welcome') }}">Administrador</a></li>
        @endif
        @if (checkRol('sstsena.funcionario'))
          <li><a class="hover:text-orange-500 transition" href="{{ route('sstsena.funcionario.welcome') }}">Funcionario</a></li>
        @endif
      @endauth   
...
<main class="relative pt-32 pb-24 bg-gradient-to-br from-white via-gray-100 to-white">
  <div class="absolute inset-0 pointer-events-none">
    <svg class="absolute top-0 left-0 w-80 h-80 opacity-10" viewBox="0 0 200 200"><circle fill="#fb923c" cx="100" cy="100" r="100"/></svg>
    <svg class="absolute bottom-0 right-0 w-64 h-64 opacity-10" viewBox="0 0 200 200"><rect fill="#60a5fa" width="200" height="200"/></svg>
  </div>
      ...
      <div class="flex flex-col sm:flex-row flex-wrap justify-center md:justify-start gap-4">
        @auth
          ...
          @if (!checkRol('sstsena.admin') && !checkRol('sstsena.funcionario'))
            <p class="text-gray-600 italic">Estás autenticado pero aún no tienes permisos asignados para acceder a reportes.</p>
          @endif
        @endauth

        @guest
          <a href="{{ route('login') }}" class="inline-flex items-center gap-2 px-6 py-3 border border-orange-500 text-orange-600 rounded-lg font-semibold hover:bg-orange-100 transition">

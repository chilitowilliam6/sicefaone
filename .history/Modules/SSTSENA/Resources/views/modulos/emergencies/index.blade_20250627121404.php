        ...
        <div class="section-content d-flex justify-content-between align-items-center py-4 px-4">
            <div class="d-flex align-items-center">
                <div class="section-icon me-4">
                    <i class="fas fa-exclamation-triangle fa-lg" style="color: #4169E1;"></i>
                </div>
                <div>
                    <h4 class="mb-1 fw-semibold text-dark">Gestión de Emergencias</h4>
                    <small class="text-muted">Sistema de registro y seguimiento</small>
                </div>
            </div>
            <div class="stats-container">
                <div class="stats-number">{{ $emergency->count() }}</div>
    ...
    <div class="px-4 py-3">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('sstsena.funcionario.emergencies.create') }}" class="corporate-btn" style="border-color: #28a745; background: #d4edda; color: #155724;">
                    <i class="bi bi-plus-circle-fill me-1"></i> Nueva Emergencia
                </a>
            </div>

            <!-- Filtro por Fecha -->
            <form method="GET" action="{{ route('sstsena.funcionario.emergencies.index') }}" class="d-flex align-items-center gap-2">
                ...
                <a href="{{ route('sstsena.funcionario.emergencies.index') }}" class="corporate-btn" style="border-color: #6c757d;">
        ...
        <table class="table table-hover mb-0 corporate-table">
            <thead>
                <tr class="corporate-header-row">
                    ...
                    <th class="corporate-th">Tipo de Emergencia</th>
                    <th class="corporate-th">Descripción de emergencia</th>
            ...
            <tbody>
                ...
                <tr class="corporate-row">
                    ...
                    <td class="corporate-cell">{{ $emergency->date_time }}</td>
                    <td class="corporate-cell">{{ $emergency->environment->name ?? 'N/A' }}</td>
                    <td class="corporate-cell">{{ $emergency->riskType->name ?? 'N/A' }}</td>
                    <td class="corporate-cell">{{ $emergency->emergencyType->name ?? 'N/A' }}</td>
                    <td class="corporate-cell">
                        @php
                            $maxLength = 100;
                        ...
                        @if($isLong)
                            <button type="button" class="btn btn-link p-0 text-primary text-decoration-none" 
                                    onclick="showDescriptionModal('{{ $emergency->id }}', `{{ addslashes($description) }}`)">
                                Ver más
                            </button>
                        @endif
                    </td>
                    ...
                    <td class="corporate-cell text-center">
                        ...
                        <button type="button" class="btn p | -0 border-0 bg-transparent" 
                                onclick="showEvidenceModal('{{ $emergency->id }}', '{{ asset('storage/evidences/' . $emergency->evidence) }}')">
                            <img src="{{ asset('storage/evidences/' . $emergency->evidence) }}" alt="Evidencia"
                    ...
                    <td class="corporate-cell midterm-ignore">
                        <div class="created-by-info">
                            <span class="fw-semibold text-dark">
                                {{ $emergency->user->person->first_name ?? 'Desconocido' }}
                                {{ $emergency->user->person->last_name ?? '' }}
                            </span>
                        </div>
                    </td>
                    <td class="corporate-cell text-center">
                        <div class="d-flex justify-content-center gap-2">
                            <a href="{{ route('sstsena.funcionario.emergencies.edit', $emergency->id) }}"
                                class="corporate-btn corporate-btn-edit">Editar</a>
                            <form action="{{ route('sstsena.funcionario.emergencies.destroy', $emergency->id) }}"
                                method="POST" onsubmit="return confirm('¿Estás seguro de eliminar esta emergencia?')">
                                @csrf
                                @method('DELETE')
                      ...
                      <td class="text-center corporate-cell">
                                            <a href="{{ route('sstsena.emergencies.responses.index', $emergencies->id) }}" class="corporate-btn">
                ...
                <tr>
                    <td colspan="10" class="text-center py-5">
                        <div class="empty-state-corporate">
                            ...
                            <h6 class="text-muted mb-2 fw-normal">No hay emergencias registradas</h6>
...
function showDescriptionModal(emergencyId, description) {
...
function showEvidenceModal(emergencyId, imageSrc) {

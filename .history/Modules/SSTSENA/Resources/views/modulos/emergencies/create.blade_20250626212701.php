...
<div class="container mt-5">
    <div class="card shadow-sm rounded-3">
        <div class="card-header bg-light">
               <h1 class="text-center  fw-bold" style="color: #1a3c6e; font-weight: 600;">Registrar Emergencia</h1>
        </div>
        <div class="card-body p-4">
            ...
            <form action="{{ route('sstsena.funcionario.emergencies.store') }}" method="POST" enctype="multipart/form-data" novalidate>
                @csrf
                ...
                <div class="mb-3">
                    <label for="emergency_types_id" class="form-label">Tipo de Emergencia <span class="text-danger">*</span></label>
                    <select 
                        name="emergency_types_id" 
                        id="emergency_types_id" 
                        class="form-select @error('emergency_types_id') is-invalid @enderror" 
                        required
                    >
                        <option value="" disabled {{ old('emergency_types_id') ? '' : 'selected' }}>Seleccione un tipo de emergencia</option>
                        @foreach($emergencyTypes as $type)
                            <option value="{{ $type->id }}" {{ old('emergency_types_id') == $type->id ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                    @error('emergency_types_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="text-end">
                    <a href="{{ route('sstsena.funcionario.emergencies.index') }}" class="btn btn-outline-secondary me-2">Cancelar</a>

        ...
        <div class="card-body p-4">
            <form action="{{ route('sstsena.admin.TypePerson.update', $typePerson->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" class="form-control border-light-subtle" value="{{ $personTypes->name }}" required>
                </div>
                <div class="mb-4">
                    <label for="description" class="form-label" style="color: #34495e; font-weight: 500;">Descripción</label>
                    <textarea name="description" id="description" class="form-control border-light-subtle" rows="4">{{$personTypes->description }}</textarea>
                </div>

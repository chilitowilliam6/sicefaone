        ...
        <div class="section-content d-flex justify-content-between align-items-center py-4 px-4">
            <div class="d-flex align-items-center">
                <div class="section-icon me-4">
                    <i class="fas fa-exclamation-triangle fa-lg" style="color: #708090;"></i>
                </div>
                <div>
                    <h4 class="mb-1 fw-semibold text-dark">Gestión de Actos Inseguros</h4>
                    <small class="text-muted">Sistema de registro y seguimiento</small>
                </div>
            </div>
            <div class="stats-container">
                <div class="stats-number">{{ $unsafe_acts->count() }}</div>
    ...
    <div class="px-4 py-3">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('sstsena.funcionario.unsafe_acts.create') }}" class="corporate-btn" style="border-color: #28a745; background: #d4edda; color: #155724;">
                    <i class="bi bi-plus-circle-fill me-1"></i> Nuevo Acto Inseguro
                </a>
            </div>

            <!-- Filtro por Fecha -->
            <form method="GET" action="{{ route('sstsena.funcionario.unsafe_acts.index') }}" class="d-flex align-items-center gap-2">
                ...
                <a href="{{ route('sstsena.funcionario.unsafe_acts.index') }}" class="corporate-btn" style="border-color: #6c757d;">
...
<div class="corporate-card shadow-sm">
        ...
        <table class="table table-hover mb-0 corporate-table">
            <thead>
                <tr class="corporate-header-row">
                    ...
                    <th class="corporate-th">Tipo de Acto Inseguro</th>
                    <th class="corporate-th">Descripción de inseguridad</th>
            ...
            <tbody>
                ...
                <tr class="corporate-row">
                    ...
                    <td class="corporate-cell">{{ $unsafe_act->date_time }}</td>
                    <td class="corporate-cell">{{ $unsafe_act->environment->name ?? 'N/A' }}</td>
                    <td class="corporate-cell">{{ $unsafe_act->riskType->name ?? 'N/A' }}</td>
                    <td class="corporate-cell">{{ $unsafe_act->unsafeActType->name ?? 'N/A' }}</td>
                    <td class="corporate-cell">
                        @php
                            $maxLength = 100;
                            $description = $unsafe_act->description;
                        ...
                        @if($isLong)
                            <button type="button" class="btn btn-link p-0 text-primary text-decoration-none" 
                                    onclick="showDescriptionModal('{{ $unsafe_act->id }}', `{{ addslashes($description) }}`)">
                                Ver más
                            </button>
                        @endif
                    </td>
                    <td class="corporate-cell">
                        @switch($unsafe_act->severity)
                            @case('minor')
                    ...
                    <td class="corporate-cell text-center">
                        ...
                        <button type="button" class="btn p-0 border-0 bg-transparent" 
                                onclick="showEvidenceModal('{{ $unsafe_act->id }}', '{{ asset('storage/' . $unsafe_act->evidence) }}')">
                            <img src="{{ asset('storage/' . $unsafe_act->evidence) }}" alt="Evidencia"
                    ...
                    <td class="corporate-cell midterm-ignore">
                        <div class="created-by-info">
                            <span class="fw-semibold text-dark">
                                {{ $unsafe_act->user->person->first_name ?? 'Desconocido' }}
                                {{ $unsafe_act->user->person->last_name ?? '' }}
                            </span>
                        </div>
                    </td>
                    <td class="corporate-cell text-center">
                        <div class="d-flex justify-content-center gap-2">
                            <a href="{{ route('sstsena.funcionario.unsafe_acts.edit', $unsafe_act->id) }}"
                                class="corporate-btn corporate-btn-edit">Editar</a>
                            <form action="{{ route('sstsena.funcionario.unsafe_acts.destroy', $unsafe_act->id) }}"
                                method="POST" onsubmit="return confirm('¿Estás seguro de eliminar este acto inseguro?')">
                                @csrf
                                @method('DELETE')
    ...
    <a href="{{ route('sstsena.unsafe_acts.responses.index', $unsafe_act->id) }}" class="corporate-btn corporate-btn-history">
                ...
                <tr>
                    <td colspan="10" class="text-center py-5">
                        <div class="empty-state-corporate">
                            ...
                            <h6 class="text-muted mb-2 fw-normal">No hay actos inseguros registrados</h6>
...
@media (max-width: 768px) {
    ...
    .corporate-th { font-size: 0.6rem; padding: 1rem 0.75rem; }
...
function showDescriptionModal(unsafeActId, description) {
...
function showEvidenceModal(unsafeActId, imageSrc) {

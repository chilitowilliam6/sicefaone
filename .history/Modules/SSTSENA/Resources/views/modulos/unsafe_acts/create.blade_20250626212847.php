...
<div class="container mt-5">
    <div class="card shadow-sm rounded-3">
        <div class="card-header bg-light">
              <h1 class="text-center  fw-bold" style="color: #1a3c6e; font-weight: 600;">Registrar Actos Inseguros</h1>
        </div>
        <div class="card-body p-4">
            ...
            <form action="{{ route('sstsena.funcionario.unsafe_acts.store') }}" method="POST" enctype="multipart/form-data" novalidate>
                @csrf
                ...
                <div class="mb-3">
                    <label for="environment_id" class="form-label">Ubicacion <span class="text-danger">*</span></label>
                    <select
                        name="environment_id"
                        id="environment_id"
                ...
                <div class="mb-3">
                    <label for="unsafe_act_type_id" class="form-label">Tipo de Acto Inseguro <span class="text-danger">*</span></label>
                    <select
                        name="unsafe_act_type_id"
                        id="unsafe_act_type_id"
                        class="form-select @error('unsafe_act_type_id') is-invalid @enderror"
                        required
                    >
                        <option value="" disabled {{ old('unsafe_act_type_id') ? '' : 'selected' }}>Seleccione un tipo de acto</option>
                        @foreach($unsafeActTypes as $type)
                            <option value="{{ $type->id }}" {{ old('unsafe_act_type_id') == $type->id ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                    @error('unsafe_act_type_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="text-end">
                    <a href="{{ route('sstsena.funcionario.unsafe_acts.index') }}" class="btn btn-outline-secondary me-2">Cancelar</a>

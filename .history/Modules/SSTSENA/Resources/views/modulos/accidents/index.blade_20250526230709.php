            ...
            <form id="accidentForm" action="{{ route('sstsena.funcionario.accidents.update', $accident->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    @if($accident->evidence)
                        <div class="mt-2">
                            <small>Evidencia actual: <a href="{{ asset('storage/'.$accident->evidence) }}" target="_blank">Ver archivo</a></small>
                        </div>
                    @endif
                </div>
                <div class="mb-4">
...
<script>
    document.getElementById('accidentForm').addEventListener('submit', function(e) {
        ...
        Swal.fire({
            ...
            text: "¿Estás seguro de que deseas actualizar este accidente?",
            icon: 'warning',

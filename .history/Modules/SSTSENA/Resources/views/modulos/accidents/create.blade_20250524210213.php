...
@endif
    ...
    <form action="{{ route('sstsena.funcionario.accidents.store') }}" method="POST" class="bg-dark text-white p-4 rounded shadow "  enctype="multipart/form-data" >
        @csrf
        ...
        <div class="mb-3">
          ...
          <div class="mb-3">
            <label for="injury_type_id" class="form-label">Entorno</label>
            <select name="environment_id" id="injury_type_id" class="form-select" required>
                <option value="" disabled selected>Seleccione un entorno</option>
                @foreach($environment as $environmet)
       ...
       <div class="mb-3">
    <label for="evidence" class="form-label">Evidencia (Imagen)</label>
    <input type="file" name="evidence" id="evidence" class="form-control" accept=".jpg,.jpeg,.png">
      </div>

...
@section('content')
    ...
    <script>
        ...
        const rawData = {
            accidents: [
                @foreach ($accidents as $accident)
                    { date_time: '{{ $accident->date_time }}' }@if (!$loop->last),@endif
                @endforeach
            ],
            incidents: [
            ...
            unsafeActs: [
                @foreach ($unsafeActs as $unsafeAct)
                    { date_time: '{{ $unsafeAct->date_time }}' }{{ !$loop->last ? ',' : '' }}
                @endforeach
            ]
        };
        ...
        function processDataByMonthYear(data, year) {
            ...
            const unsafeActsByMonth = new Array(12).fill(0);
            ...
            // Count unsafe acts
            data.unsafeActs.forEach(item => {
                if (item.date_time) {
                    const date = moment(item.date_time);
                    if (date.isValid() && date.year() === parseInt(year)) {
                        unsafeActsByMonth[date.month()]++;
                    }
                }
            });

            return { months, accidentsByMonth, incidentsByMonth, emergenciesByMonth, unsafeActsByMonth };
        }
        ...
        function updateChart() {
            ...
            const { months, accidentsByMonth, incidentsByMonth, emergenciesByMonth, unsafeActsByMonth } = processDataByMonthYear(rawData, selectedYear);

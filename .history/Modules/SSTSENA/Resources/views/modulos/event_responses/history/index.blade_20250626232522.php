        ...
        <div class="col-12 section-item">
            ...
            <div class="corporate-card border-0 shadow-sm">
                ...
                <div class="section-header" style="border-left: 4px solid #FF8C00;">
                    <div class="section-content d-flex justify-content-between align-items-center py-4 px-4">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <h4 class="mb-1 fw-semibold text-dark">Respuestas a Todos los Eventos SST</h4>
                                <small class="text-muted">Gestión de Respuestas</small>
                            </div>
                        </div>
                        <div class="stats-container">
                            <div class="stats-number">
                                {{ $accidentResponses->count() + $incidentResponses->count() + $emergencyResponses->count() + $unsafeActResponses->count() }}
                            </div>
                ...
                <div class="px-4 py-3">
                    <div class="d-flex align-items-end mb-3 gap-3 justify-content-between">
                        <div class="d-flex align-items-end gap-3">
                            ...
                            <a href="{{ route('sstsena.funcionario.event_responses.history.index') }}" class="corporate-btn" style="border: 2px solid #adb5bd;">
                                <i class="fas fa-times me-2"></i>
                                <span>Limpiar Filtro</span>
                            </a>
                        </div>
                        <a href="{{ route('events.index') }}" class="corporate-btn corporate-btn-delete" style="border-color: #dc3545;">
                            <i class="fas fa-arrow-left me-2"></i>
                            <span>Volver</span>
                        </a>
                    </div>
                    <form id="dateFilterForm" method="GET" action="{{ route('sstsena.funcionario.event_responses.history.index') }}" class="d-none">
                ...
                <div class="table-container">
                        ...
                        <table class="table table-hover mb-0 corporate-table">
                            <thead>
                                <tr class="corporate-header-row">
                                    ...
                                    <th class="corporate-th midterm-ignore" style="width: 15%;">
                                        <div class="th-content">
                            ...
                            <tbody>
                                @php
                                    $tipos = [
                                        ['titulo' => 'Accidentes', 'items' => $accidentResponses, 'route' => 'sstsena.accidents.responses'],
                                        ['titulo' => 'Incidentes', 'items' => $incidentResponses, 'route' => 'sstsena.incidents.responses'],
                                        ['titulo' => 'Emergencias', 'items' => $emergencyResponses, 'route' => 'sstsena.emergencies.responses'],
                                        ['titulo' => 'Actos Inseguros', 'items' => $unsafeActResponses, 'route' => 'sstsena.unsafeActs.responses'],
                                    ];
                                    $allResponses = collect([]);
                                    foreach ($tipos as $tipo) {
                                        $allResponses = $allResponses->merge($tipo['items']->map(function($item) use ($tipo) {
                                            return ['response' => $item, 'tipo' => $tipo['titulo'], 'route' => $tipo['route']];
                                        }));
                                    }
                                    $allResponses = $allResponses->sortByDesc(function($item) {
                                        return $item['response']->created_at;
                                    });
                                @endphp

                                @forelse ($allResponses as $item)
                                    @php
                                    ...
                                    <tr class="corporate-row">
                                        ...
                                        <td class="corporate-cell midterm-ignore">
                                            <div class="datetime-info">
                                                ...
                                                <small class="text-muted">
                                                    <i class="fas fa-clock me-1"></i>{{ \Carbon\Carbon::parse($response->created_at)->format('H:i') }}
                                                </small>
                                            </div>
                                        </td>
                                        <td class="corporate-cell midterm-ignore">
                                        ...
                                        <td class="corporate-cell midterm-ignore">
                                            <span class="fw-semibold text-dark">{{ $tipo }}</span>
                                        </td>
                                        <td class="text-center corporate-cell midterm-ignore">
                                            <div class="d-flex justify-content-center gap-2">
                                                <a href="{{ route($route . '.edit', [$response->event_id, $response->id]) }}"
                                                   class="corporate-btn corporate-btn-edit" style="border-color: #28a745;">
                                                    <i class="fas fa-edit"></i>
                                                    <span>Editar</span>
                                                </a>
                                                <form action="{{ route($route . '.destroy', [$response->event_id, $response->id]) }}"
                    ...
                    @if($allResponses instanceof \Illuminate\Pagination\LengthAwarePaginator)
                        <div class="mt-3 px-4">
                            {{ $allResponses->links() }}
                        </div>
                    @endif

...
<div class="row g-4 mb-4">
    <div class="col-12 section-item">
     ...
     {{-- Filtro por fecha del accidente --}}
<div class="filter-container px-4 py-3 border-bottom">
    <form method="GET" action="{{ request()->url() }}" class="row g-3 align-items-end">
        <div class="col-md-3">
            <label for="date_time" class="form-label small fw-semibold text-dark">Fecha del Accidente</label>
            <input type="date" name="date_time" id="date_time" class="form-control form-control-sm" value="{{ request()->input('date_time') }}">
        </div>
        <div class="col-md-3">
                                        ...
                                        <div class="evidence-container">
                                            @if($item->evidence)
                                                @php
                                                    ...
                                                    $evidenceAlt = "Evidencia del accidente #" . $item->id;
                                                @endphp
                                                    ...
                                                    <div class="evidence-preview">
                                                        ...
                                                        <div class="evidence-actions mt-2">
                                                            <a href="{{ asset('storage/evidences/' . $item->evidence) }}"
                                                               ...
                                                               download="{{ 'evidencia-accidente-' . $item->id . '.' . $ext }}">
...
<div class="row g-4 mb-4">
    <div class="col-12 section-item">
        <div class="corporate-card border-0 shadow-sm">
            {{-- Encabezado de la sección corporativa --}}
            ...
            <div class="filter-container px-4 py-3">
                <form method="GET" action="{{ route('sstsena.incidentes.respuestas.index') }}">
                    <div class="row align-items-end">
                        <div class="col-md-4 col-lg-3">
                            <label for="fecha_hora" class="form-label fw-semibold">Filtrar por fecha</label>
                            <input type="date" name="fecha_hora" id="fecha_hora" class="form-control" value="{{ request()->input('fecha_hora') }}">
                        </div>
                        ...
                        <div class="col-md-2 col-lg-2 mt-3 mt-md-0">
                            <a href="{{ route('sstsena.incidentes.respuestas.index') }}" class="btn btn-secondary w-100">
                    ...
                    <table class="table table-hover mb-0 corporate-table">
                        <thead>
                            <tr class="corporate-header-row">
                                ...
                                <th class="corporate-th"><i class="fas fa-image me-2"></i>Evidencia</th>
                        ...
                        <tbody>
                            @forelse ($incidents as $item)
                                <tr class="corporate-row">
                                    ...
                                    <td class="corporate-cell">
                                        <div class="fw-semibold texto-oscuro">{{ $item->fecha_hora->format('d M Y') }}</div>
                                        <small class="text-muted"><i class="fas fa-clock me-1"></i>{{ $item->fecha_hora->format('H:i') }}</small>
                                    </td>
                                    <td class="corporate-cell">
                                    ...
                                    <td class="corporate-cell">
                                        <span class="fw-semibold text-dark">{{ $item->incidentType->name ?? 'N/D' }}</span>
                                    </td>
                                    <td class="corporate-cell">
                                        <p class="mb-0 text-dark lh-sm expandable-text" id="description-incidents-{{ $item->id }}" style="max-height: 3em; overflow: hidden; transition: max-height 0.3s ease;">
                                            {{ $item->description ?? 'Sin descripción disponible' }}
                                        </p>
                                        @if(strlen($item->description ?? '') > 65)
                                        ...
                                        @if($item->evidence)
                                            ...
                                            @if($isImage)
                                                <a href="#" data-bs-toggle="modal" data-bs-target="#{{ $modalId }}">
                                                    <img src="{{ $evidencePath }}" alt="Evidencia" class="img-thumbnail shadow-sm" style="width: 60px; height: 60px; object-fit: cover;">
                                                </a>
                                                <div class="mt-1">
                                                    <a href="{{ $evidencePath }}" download class="btn btn-light border d-inline-flex align-items-center gap-1 px-2 py-0 small rounded" style="font-size: 0.7rem; line-height: 1rem;" title="Descargar imagen">
                                                        <i class="fas fa-download" style="font-size: 0.75rem;"></i> Descargar
                                                    </a>
                                                </div>
                                                    ...
                                                    <div class="modal-dialog modal-dialog-centered modal-lg">
                                                        <div class="modal-content rounded-3">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title fw-semibold" id="{{ $modalId }}Etiqueta">Evidencia del Incidente</h5>
                                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                                                            </div>
                                                            <div class="modal-body text-center">
                                                                <img src="{{ $evidencePath }}" alt="Evidencia" class="img-fluid rounded shadow" style="max-height: 600px; object-fit: contain;">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            @else
                                                <a href="{{ $evidencePath }}" download class="btn btn-light border d-inline-flex align-items-center gap-1 px-2 py-0 small rounded mt-1" style="font-size: 0.7rem; line-height: 1rem;" title="Descargar archivo">
                                                    <i class="fas fa-download" style="font-size: 0.75rem;"></i> Descargar
                                                </a>
                                            @endif
                                        @else
                                    ...
                                    <td class="text-center corporate-cell">
                                        <a href="{{ route('sstsena.incidentes.respuestas.index', $item->id) }}" class="corporate-btn">
                                            <i class="fas fa-folder-open me-2"></i>
                                            <span>Revisar caso</span>
                                        </a>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="11" class="text-center py-5">
                                        <div class="empty-state-corporate">
                                            ...
                                            <h6 class="text-muted mb-2 fw-normal">No se encontraron registros de incidentes laborales</h6>
                            ...
                            @forelse ($emergencies as $item)
                                <tr class="corporate-row">
                                    ...
                                    <td class="corporate-cell text-center align-middle">
                                     ...
                                     {{-- Creado por --}}
                                          <td class="célula corporativa accidentes-célula-extra" style="ancho: 8%;">
                                     <div class="creado-por-info">
                                         <span class="fw-semibold text-dark">{{ $item->user->person->first_name ?? 'Funcionario' }}</span>
                                         @if($item->user)
                                             <small class="text-muted d-block">{{ $item->user->email ?? '' }}</small>

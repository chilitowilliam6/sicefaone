                        ...
                        @else
                            @if ($section['title'] == 'Accidentes Laborales')
                                <div class="slider-wrapper">
                                    <button class="slider-nav prev" data-section="{{ $index }}" aria-label="Mostrar grupo anterior de accidentes">
                                    ...
                                    <button class="slider-nav next" data-section="{{ $index }}" aria-label="Mostrar siguiente grupo de accidentes">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            document.querySelectorAll('.slider-wrapper').forEach(wrapper => {
                const sectionIndex = wrapper.querySelector('.slider-nav').dataset.section;
                const slider = document.getElementById(`slider-${sectionIndex}`);
                const inner = slider.querySelector('.slider-inner');
                const cards = inner.querySelectorAll('.card-col');
...
.card-col {
    flex: 0 0 calc(33.333% - 1rem); /* Three cards per group with gap */
...
@media (max-width: 992px) {
    .card-col {
        flex: 0 0 calc(50% - 1rem); /* Two cards per group on medium screens */
    }
}

@media (max-width: 576px) {
    .card-col {
        flex: 0 0 calc(100% - 1rem); /* One card per group on small screens */
    }

...
<div class="container-fluid py-4">
    ...
    @if (session('success'))
        ...
        <script>setTimeout(() => document.getElementById('alertaExito')?.classList.add('fade-out'), 5000);
    ...
    document.querySelectorAll('.toggle-description').forEach(function (element) {
        element.addEventListener('click', function () {
            ...
            if (isExpanded) {
                descriptionElement.textContent = fullText.length > 65 ? fullText.substring(0, 65) + '...' : fullText;
    ...
    <div class="description-content">
        <p class="mb-0 text-dark lh-sm description-text" id="description-{{ $item->id }}">
            {{ Str::limit($item->description ?? 'Sin descripción disponible', 65) }}
        </p>
        @if(strlen($item->description ?? '') > 65)
            <small class="text-muted mt-1 d-block">
                <a href="javascript:void(0);" class="toggle-description text-decoration-none" data-id="{{ $item->id }}" data-full-text="{{ addslashes($item->description ?? 'Sin descripción disponible') }}">
                    <i class="fas fa-external-link-alt me-1"></i><span>Ver más</span>
                </a>
            </small>
        @endif
    </div>

                                                ...
                                                <div class="modal-content corporate-modal">
                                                    ...
                                                    <div class="modal-footer border-0">
                                                        <div class="export-buttons me-auto">
                                                            ...
                                                            <button class="btn corporate-btn export-btn" onclick="exportToExcel('modalContent{{ $item->id }}', '{{ $section['title'] }}_{{ str_pad($item->id, 4, '0', STR_PAD_LEFT) }}')">
...
@section('scripts')
    ...
    <script>
        ...
        function exportToExcel(elementId, filename) {
            ...
            const data = [
                ['Campo', 'Valor'],
                ['Fecha y Hora', element.querySelector('.modal-body > div:nth-child(1) p').textContent.trim()],
                ['Severidad', element.querySelector('.modal-body > div:nth-child(2) span span:last-child').textContent.trim()],
                ['Descripción', element.querySelector('.modal-body > div:nth-child(3) p').textContent.trim()],
                ['Respuestas', element.querySelector('.modal-body > div:nth-child(4) p').textContent.trim()]
            ];
            
            if (element.querySelector('.modal-body > div:nth-child(5)')) {
                const additionalInfo = element.querySelectorAll('.modal-body > div:nth-child(5) .info-item span');
                const labels = ['Ambiente', 'Tipo de Lesión', 'Tipo de Riesgo', 'Tipo de Accidente', 'Creado Por'];
                additionalInfo.forEach((item, index) => {
                    data.push([labels[index], item.textContent.trim()]);
                });
            }
...
<style>
/* Existing styles remain unchanged, adding modal-specific styles */
.corporate-modal {
    background: white;
    border-radius: 8px;

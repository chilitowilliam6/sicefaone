                ...
                <div class="card border-0 shadow-sm hover-card">
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover mb-0 professional-table align-middle">
                                <thead class="table-light text-dark text-uppercase small">
                                    <tr>
                                        <th class="fw-semibold border-0 py-3 px-3">ID</th>
                                        <th class="fw-semibold border-0 py-3 px-3">Fecha y Hora</th>
                                        <th class="fw-semibold border-0 py-3 px-3">Descripción</th>
                                        <th class="fw-semibold border-0 py-3 px-3">Severidad</th>
                                        <th class="fw-semibold border-0 py-3 px-3 text-center">Respuestas</th>
                                        <th class="fw-semibold border-0 py-3 px-3 text-center">Gestión</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse ($section['items'] as $item)
                                        <tr class="border-bottom">
                                            ...
                                            <td class="py-3 px-3 text-muted">
                                                {{ $item->date_time->format('d/m/Y') }}<br>
                                                <small class="text-secondary">{{ $item->date_time->format('H:i:s') }}</small>
                                            </td>
                                            <td class="py-3 px-3 text-dark">
                                                {{ Str::limit($item->description ?? 'Sin descripción disponible', 60) }}
                                            </td>
                                            <td class="py-3 px-3">
                                                @switch($item->severity)
                                                    @case('minor') 
                                                        <span class="badge bg-success bg-opacity-25 text-success">Leve</span> 
                                                    @break
                                                    @case('moderate') 
                                                        <span class="badge bg-warning bg-opacity-25 text-warning">Moderada</span> 
                                                    @break
                                                    @case('serious') 
                                                        <span class="badge bg-danger bg-opacity-25 text-danger">Grave</span> 
                                                    @break
                                                    @case('fatal') 
                                                        <span class="badge bg-dark bg-opacity-25 text-dark">Crítica</span> 
                                                    @break
                                                    @default 
                                                        <span class="badge bg-secondary bg-opacity-25 text-secondary">Por evaluar</span>
                                                @endswitch
                                            </td>
                                            ...
                                            <td class="text-center py-3 px-3">
                                                <a href="{{ route('sstsena.' . $section['route_prefix'] . '.responses.index', $item->id) }}" 
                                                   class="btn btn-sm btn-outline-secondary px-3 py-2 professional-btn">

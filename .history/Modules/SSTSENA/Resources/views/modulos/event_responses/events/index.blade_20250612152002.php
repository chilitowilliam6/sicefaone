    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content corporate-modal">
            ...
            <div class="modal-body text-center p-0">
                @if(file_exists(public_path('storage/evidences/' . $item->evidence)))
                    <img src="{{ asset('storage/evidences/' . $item->evidence) }}"
                         alt="Evidencia del accidente #{{ str_pad($item->id, 4, '0', STR_PAD_LEFT) }}"
                         class="img-fluid rounded corporate-modal-image"
                         style="max-height: 80vh; width: 100%; object-fit: contain;"
                         onerror="this.onerror=null; this.src='{{ asset('images/fallback-image.png') }}'; this.alt='Imagen no disponible';"
                         loading="lazy">
                @else
                    <div class="alert alert-warning m-3">
                        <i class="fas fa-exclamation-triangle me-2"></i>
                        La imagen no está disponible o no se encuentra en el servidor.
                    </div>
                @endif
            </div>

...
<td class="corporate-cell">
    <div class="description-content">
        ...
        @if(strlen($response->response) > 65)
            <small class="text-muted mt-1 d-block" data-bs-toggle="modal" data-bs-target="#modalResponse{{ $response->id }}">
                <i class="fas fa-external-link-alt me-1"></i>Ver detalles completos
            </small>

            <!-- Modal para respuesta completa -->
            <div class="modal fade" id="modalResponse{{ $response->id }}" tabindex="-1" aria-labelledby="modalResponseLabel{{ $response->id }}" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="modalResponseLabel{{ $response->id }}">Detalle completo de la respuesta</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                        </div>
                        <div class="modal-body">
                            <p class="text-dark">{{ $response->response }}</p>
                        </div>
                    </div>
                </div>
            </div>
...
<!-- Celda: Acciones Tomadas -->
<td class="corporate-cell">
    <div class="description-content">
        <p class="mb-0 text-dark lh-sm">{{ Str::limit($response->actions_taken, 65) }}</p>

        @if(strlen($response->actions_taken) > 65)
            <small class="text-muted mt-1 d-block" data-bs-toggle="modal" data-bs-target="#modalAction{{ $response->id }}">
                <i class="fas fa-external-link-alt me-1"></i>Ver detalles completos
            </small>

            <!-- Modal para acciones tomadas -->
            <div class="modal fade" id="modalAction{{ $response->id }}" tabindex="-1" aria-labelledby="modalActionLabel{{ $response->id }}" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="modalActionLabel{{ $response->id }}">Detalle completo de las acciones</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                        </div>
                        <div class="modal-body">
                            <p class="text-dark">{{ $response->actions_taken }}</p>
                        </div>
                    </div>
                </div>
            </div>

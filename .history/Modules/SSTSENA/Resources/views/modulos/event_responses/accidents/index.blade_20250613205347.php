    ...
    <div class="row g-4">
        <div class="col-12 section-item">
            ...
            <div class="corporate-card border-0 shadow-sm">
                ...
                {{-- Botones fuera de la tabla, alineados a la izquierda --}}
                <div class="px-4 py-3 d-flex align-items-center gap-3">
                    ...
                    <form id="bulk-delete-form" action="{{ route('sstsena.accidents.responses.bulkDestroy', $event->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" id="bulk-delete-btn" class="corporate-btn" disabled
                        ...
                        <table class="table table-hover mb-0 corporate-table">
                            <thead>
                                <tr class="corporate-header-row">
                                    <th class="corporate-th" style="width: 50px;">
                                    ...
                                    <tr class="corporate-row">
                                        <td class="corporate-cell text-center">
                                            <input type="checkbox" name="response_ids[]" value="{{ $response->id }}"
                                                   class="response-checkbox" form="bulk-delete-form">
                                        </td>
                                        ...
                                        <td class="corporate-cell">
                                            <div class="reference-id">
                                                <span class="fw-bold" style="color: #DC143C;">
                                                    #{{ str_pad($response->id, 4, '0', STR_PAD_LEFT) }}
                                                </span>
                                            </div>
                                        </td>
                                        <td class="corporate-cell">
                                        ...
                                        <td class="corporate-cell">
                                            <div class="created-by-info">
                                                <span class="fw-semibold text-dark">{{ $response->user->nickname ?? 'Administrador' }}</span>
                                                @if($response->user)
                                                    <small class="text-muted d-block">{{ $response->user->correo_electronico ?? '' }}</small>
                                                @endif
                                            </div>
                                        </td>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        responseCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                } else if (document.querySelectorAll('.response-checkbox:checked').length === responseCheckboxes.length) {
        ...
        function toggleBulkDeleteButton() {
            bulkDeleteBtn.disabled = document.querySelectorAll('.response-checkbox:checked').length === 0;
        }

        function confirmBulkDelete() {
            const checkedCount = document.querySelectorAll('.response-checkbox:checked').length;
            if (checkedCount === 0) {
                Swal.fire({
                    title: 'Error',
                    text: 'No has seleccionado ningún registro para eliminar.',
                    icon: 'error',
            ...
            return Swal.fire({
                title: '¿Estás seguro?',
                text: `¿Deseas eliminar ${checkedCount} registro(s) seleccionado(s)?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        title: 'Eliminado!',
                        text: 'Los registros seleccionados han sido eliminados.',
                        icon: 'success',
                        confirmButtonColor: '#DC143C',
                        timer: 3000,
        ...
        // Mostrar confirmación al enviar formularios individuales
        const forms = document.querySelectorAll('form:not(#bulk-delete-form)');

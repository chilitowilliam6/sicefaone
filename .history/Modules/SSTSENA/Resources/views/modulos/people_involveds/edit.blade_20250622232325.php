...
<div class="container mt-5">
    <div class="card shadow-sm rounded-3" style="background-color: #ffffff;">
        ...
        <div class="card-body p-4">
            <form action="{{ route('sstsena.funcionario.people_involved.update', $people->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="document_type" id="document_type" class="form-control border-light-subtle" required>
                        <option value="CC" {{ $people->document_type == 'CC' ? 'selected' : '' }}>Cédula de Ciudadanía</option>
                        <option value="TI" {{ $people->document_type == 'TI' ? 'selected' : '' }}>Tarjeta de Identidad</option>
                        <option value="CE" {{ $people->document_type == 'CE' ? 'selected' : '' }}>Cédula de Extranjería</option>
                        <option value="PAS" {{ $people->document_type == 'PAS' ? 'selected' : '' }}>Pasaporte</option>
                        <option value="OTRO" {{ $people->document_type == 'OTRO' ? 'selected' : '' }}>Otro</option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="document_number" id="document_number" class="form-control border-light-subtle" value="{{ $people->document_number }}" required>
                </div>

                {{-- Nombre --}}
                <div class="mb-4">
                    <label for="name" class="form-label" style="color: #34495e; font-weight: 500;">Nombre</label>
                    <input type="text" name="name" id="name" class="form-control border-light-subtle" value="{{ $people->name }}" required>
                </div>

                {{-- Apellido --}}
                <div class="mb-4">
                    <label for="last_name" class="form-label" style="color: #34495e; font-weight: 500;">Apellido</label>
                    <input type="text" name="last_name" id="last_name" class="form-control border-light-subtle" value="{{ $people->last_name }}" required>
                </div>

                {{-- Fecha de Nacimiento --}}
                <div class="mb-4">
                    <label for="birth_date" class="form-label" style="color: #34495e; font-weight: 500;">Fecha de Nacimiento</label>
                    <input type="date" name="birth_date" id="birth_date" class="form-control border-light-subtle" value="{{ $people->birth_date }}" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="gender" id="gender" class="form-control border-light-subtle" required>
                        ...
                        <option value="Masculino" {{ $people->gender == 'Masculino' ? 'selected' : '' }}>Masculino</option>
                        <option value="Femenino" {{ $people->gender == 'Femenino' ? 'selected' : '' }}>Femenino</option>
                        <option value="Otro" {{ $people->gender == 'Otro' ? 'selected' : '' }}>Otro</option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="person_type_id" id="person_type_id" class="form-control border-light-subtle" required>
                        ...
                        @foreach ($typePersons as $type)
                            <option value="{{ $type->id }}" {{ $people->person_type_id == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="phone" id="phone" class="form-control border-light-subtle" value="{{ $people->phone }}">
                </div>

                {{-- Dirección --}}
                <div class="mb-4">
                    <label for="address" class="form-label" style="color: #34495e; font-weight: 500;">Dirección</label>
                    <input type="text" name="address" id="address" class="form-control border-light-subtle" value="{{ $people->address }}">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="accident_id" id="accident_id" class="form-control border-light-subtle" required>
                        ...
                        @foreach ($accident as $accident)
                            <option value="{{ $accident->id }}" {{ $people->accident_id == $accident->id ? 'selected' : '' }}>

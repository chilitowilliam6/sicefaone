...
<div class="mb-4 text-center">
    <label for="accident_id" class="form-label d-block mb-2" style="color: #34495e; font-weight: 500;">Accidente Asociado</label>
    <div class="d-flex justify-content-center">
        <select name="accident_id" id="accident_id"
                class="form-select border-light-subtle w-100"
            ...
            @foreach ($accident as $acc)
                <option value="{{ $acc->id }}" title="{{ $acc->description }}">
                    {{ $acc->id }} - {{ Str::limit($acc->description, 50, '...') }}
                </option>
            @endforeach
        </select>
    </div>

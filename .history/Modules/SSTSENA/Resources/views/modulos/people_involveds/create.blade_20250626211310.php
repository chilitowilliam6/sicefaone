...
<div class="container mt-5">
    <div class="card shadow-sm rounded-3" style="background-color: #ffffff;">
        <div class="card-header" style="background-color: #f8f9fa; border-bottom: 1px solid #dee2e6;">
            <h3 class="text-center" style="color: #1a3c6e; font-weight: 600;">Registrar Persona Involucrada</h3>
        </div>
...
<div class="mb-4">
    ...
    <select name="accident_id" id="accident_id"
            class="form-control border-light-subtle text-truncate" required>
        <option value="">Seleccione un accidente</option>
        @foreach ($accident as $acc)
            <option value="{{ $acc->id }}" title="{{ $acc->description }}">
                {{ $acc->id }} - {{ \Illuminate\Support\Str::limit($acc->description, 50, '...') }}
            </option>
        @endforeach
    </select>
</div>
            ...
            <button type="submit" class="btn btn-primary px-4" style="background-color: #1a3c6e; border-color: #1a3c6e;">Registrar</button>
            </form>
        </div>
    </div>
</div>

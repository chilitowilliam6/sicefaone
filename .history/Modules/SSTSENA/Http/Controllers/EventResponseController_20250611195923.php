...
class EventResponseController extends Controller
{
    ...
    {
        $event = Accidents::findOrFail($accidentId);
        $responses = $event->eventResponses()->with('createdBy')->get();
        return view('sstsena::event_responses.accidents.index', compact('event', 'responses'));
    }

    public function createAccidents($accidentId)
    {
        $event = Accidents::findOrFail($accidentId);
        return view('sstsena::event_responses.accidents.create', compact('event'));
    }
    ...
    public function editAccidents($accidentId, $id)
    {
        ...
        return view('sstsena::event_responses.accidents.edit', compact('event', 'response'));
    }
    ...
    {
        $event = Incident::findOrFail($incidentId);
        $responses = $event->eventResponses()->with('createdBy')->get();
        return view('sstsena::event_responses.incidents.index', compact('event', 'responses'));
    }

    public function createIncidents($incidentId)
    {
        $event = Incident::findOrFail($incidentId);
        return view('sstsena::event_responses.incidents.create', compact('event'));
    }
    ...
    public function editIncidents($incidentId, $id)
    {
        $event = Incident::findOrFail($incidentId);
        $response = EventResponse::findOrFail($id);
        return view('sstsena::event_responses.incidents.edit', compact('event', 'response'));
    }
    ...
    public function indexEmergencies($emergencyId)
    {
        ...
        $responses = $event->eventResponses()->with('createdBy')->get();
        return view('sstsena::event_responses.emergencies.index', compact('event', 'responses'));
    }

    public function createEmergencies($emergencyId)
    {
        $event = Emergency::findOrFail($emergencyId);
        return view('sstsena::event_responses.emergencies.create', compact('event'));
    }
    ...
    public function editEmergencies($emergencyId, $id)
    {
        $event = Emergency::findOrFail($emergencyId);
        $response = EventResponse::findOrFail($id);
        return view('sstsena::event_responses.emergencies.edit', compact('event', 'response'));
    }
    ...
    public function indexUnsafeActs($unsafeActId)
    {
        ...
        $responses = $event->eventResponses()->with('createdBy')->get();
        return view('sstsena::event_responses.unsafe_acts.index', compact('event', 'responses'));
    }

    public function createUnsafeActs($unsafeActId)
    {
        $event = UnsafeAct::findOrFail($unsafeActId);
        return view('sstsena::event_responses.unsafe_acts.create', compact('event'));
    }
    ...
    public function editUnsafeActs($unsafeActId, $id)
    {
        $event = UnsafeAct::findOrFail($unsafeActId);
        $response = EventResponse::findOrFail($id);
        return view('sstsena::event_responses.unsafe_acts.edit', compact('event', 'response'));
    }

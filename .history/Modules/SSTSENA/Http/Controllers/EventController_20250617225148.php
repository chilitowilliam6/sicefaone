  ...
  public function index()
{
    ...
    // Aplica el filtro de fecha de creación si está presente
    if (request()->has('created_at') && !empty(request()->input('created_at'))) {
        $createdAt = request()->input('created_at');
        // Filtra los accidentes por la fecha especificada (solo la parte de la fecha)
        $accidentsQuery->whereDate('created_at', $createdAt);
    }

    // Obtiene los accidentes filtrados
    $accidents = $accidentsQuery->get();

    // Obtiene los otros modelos (sin cambios)

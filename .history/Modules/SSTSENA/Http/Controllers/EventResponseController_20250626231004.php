    ...
    public function indexAllResponses(Request $request)
{
    ...
    // Consultar todas las respuestas agrupadas por tipo de evento
    $accidentResponses = Accident::with(['eventResponses' => function ($query) use ($dateFilter, $withCreatedBy) {
        if ($dateFilter) {
            $query->whereDate('response_date', $dateFilter);
        }
        $query->with($withCreatedBy)->orderBy('response_date', 'desc');
    }])->get();

    $incidentResponses = Incidents::with(['eventResponses' => function ($query) use ($dateFilter, $withCreatedBy) {
        if ($dateFilter) {
            $query->whereDate('response_date', $dateFilter);
        }
        $query->with($withCreatedBy)->orderBy('response_date', 'desc');
    ...
    $emergencyResponses = Emergency::with(['eventResponses' => function ($query) use ($dateFilter, $withCreatedBy) {
        if ($dateFilter) {
            $query->whereDate('response_date', $dateFilter);
        }
        $query->with($withCreatedBy)->orderBy('response_date', 'desc');
    ...
    $unsafeActResponses = UnsafeAct::with(['eventResponses' => function ($query) use ($dateFilter, $withCreatedBy) {
        if ($dateFilter) {
            $query->whereDate('response_date', $dateFilter);
        }
        $query->with($withCreatedBy)->orderBy('response_date', 'desc');
    ...
    return view('sstsena::modulos.event_responses.emergencies.index', compact(
        'accidentResponses',
        'incidentResponses',
        'emergencyResponses',
        'unsafeActResponses',

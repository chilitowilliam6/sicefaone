  ...
  public function indexEmergencies($emergencyId)
{
   ...
   public function indexUnsafeActs($unsafeActId)
    {
        // Busca el acto inseguro por ID, lanza un error 404 si no existe
        $event = UnsafeAct::findOrFail($unsafeActId);
        
        // Inicia una consulta para obtener las respuestas del evento con la relación 'createdBy'
        $query = $event->eventResponses()->with('createdBy');
        
        // Verifica si se proporcionó un filtro de fecha en la solicitud
        if ($dateFilter = request('date_filter')) {
            // Intenta parsear la fecha para evitar errores de formato
            try {
                // Convierte la fecha al inicio del día para comparar solo la fecha
                $date = Carbon::parse($dateFilter)->startOfDay();
                // Filtra las respuestas por la fecha de creación
                $query->whereDate('created_at', $date);
            } catch (\Exception $e) {
                // Maneja fechas inválidas (opcional, puede registrarse en logs)
                // Por ahora, simplemente ignora la fecha inválida
            }
        }
        
        // Ejecuta la consulta y obtiene las respuestas filtradas
        $responses = $query->get();
        
        // Retorna la vista con las variables 'event' y 'responses'

...
class UnsafeActController extends Controller
{
    ...
    {
        // Obtener el parámetro de fecha desde la solicitud, si existe
        $fecha = request('fecha');

        // Crear una consulta base para los actos inseguros
        $query = UnsafeAct::query();

        // Si se proporciona una fecha, agregar el filtro por fecha
        if ($fecha) {
            // Filtra los actos inseguros donde la fecha de 'date_time' coincide con la fecha proporcionada
            $query->whereDate('date_time', $fecha);
        }

        // Obtener todos los actos inseguros que cumplen con los criterios de la consulta
        $unsafe_acts = $query->get();

        // Retornar la vista con los datos de los actos inseguros

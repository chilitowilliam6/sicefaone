...
class AccidentTypeController extends Controller
{
    ...
    public function index()
    {
        $AccidentTypes = AccidentType::all();
         $environments = Environment::all(); // o como lo tengas definido
        return view('sstsena::modulos.accident_types.index', compact('AccidentTypes', 'environments'));
    ...
    {
        return view('sstsena::modulos.accidents.create');
    }
    ...
    {
        $request->validate([
            'name' => 'required|string|max:255|unique:accident_types,name,' . $id,
            'description' => 'nullable|string|max:255',
        ]);

        // Update the accident type in the database
        $AccidentType = AccidentType::findOrFail($id);
        $AccidentType->name = $request->input('name');
        $AccidentType->description = $request->input('description');
        $AccidentType->save();

        return redirect()->route('sstsena.admin.accident_types.index')->with('success', 'Accident type updated successfully.');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('sstsena.admin.accident_types.index')->with('success', 'Accident type deleted successfully.');
    }
}

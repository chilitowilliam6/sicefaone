    ...
    public function update(Request $request, $id)
{
    ...
    // 2. Buscar el accidente que se va a actualizar
    $accident = Accident::findOrFail($id);

    // 3. Actualizar los campos con los valores del formulario
    $accident->fecha = $request->input('fecha');
    $accident->descripcion = $request->input('descripcion');
    $accident->environment_id = $request->input('environment_id');
    $accident->injury_type_id = $request->input('injury_type_id');
    $accident->risk_type_id = $request->input('risk_type_id');
    $accident->accident_type_id = $request->input('accident_type_id');

    // 4. Guardar los cambios en la base de datos
    $accident->save();

    // 5. Redireccionar con un mensaje de éxito
    return redirect()->route('sstsena::modulos.accidents.index')->with('success', 'El accidente se actualizó correctamente.');
}

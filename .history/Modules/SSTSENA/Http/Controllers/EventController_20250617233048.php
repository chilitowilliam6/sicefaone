 ...
 public function index()
{
    ...
    if (request()->has('fecha_hora') && !empty(request()->input('fecha_hora'))) {
        ...
        $incidentsQuery->whereDate('date_time', $fechaHora); // Cambiado de 'fecha_hora' a 'date_time'
    }

    // Aplica el filtro de fecha para emergencias si está presente
    if (request()->has('emergency_date') && !empty(request()->input('emergency_date'))) {
        $emergencyDate = request()->input('emergency_date');
        $emergenciesQuery->whereDate('date_time', $emergencyDate);
    }

    // Aplica el filtro de fecha para actos inseguros si está presente
    if (request()->has('unsafe_act_date') && !empty(request()->input('unsafe_act_date'))) {
        $unsafeActDate = request()->input('unsafe_act_date');
        $unsafeActsQuery->whereDate('date_time', $unsafeActDate); // Cambiado de 'fecha_hora' a 'date_time'
    }

...
class PermissionsTableSeeder extends Seeder
{
  ...
  $app = App::where('slug', 'sstsena')->first();
        $permissions_admin = [];

        // 👉 Permiso del dashboard admin
        $permission = Permission::updateOrCreate(['slug' => 'sstsena.admin.welcome'], [
        ...
        // 👉 Otro permiso del admin (por ejemplo, tipos de lesiones)
        $permission = Permission::updateOrCreate(['slug' => 'sstsena.admin.injury_types.index'], [
            'name' => 'Vista principal de tipos de lesiones',
            'description' => 'Vista principal de tipos de lesiones',
            'description_english' => 'Injury Types Index View',
            'app_id' => $app->id
        ]);
        $permissions_admin[] = $permission->id;

        // ✅ 🔁 Aquí agregas los permisos del funcionario
        $funcionarioSlugs = [
            'sstsena.funcionario.people_involved.edit',
            'sstsena.funcionario.people_involved.update',
            'sstsena.funcionario.people_involved.destroy',
        ];

        $funcionarioPermissions = Permission::whereIn('slug', $funcionarioSlugs)->pluck('id')->toArray();
        $permissions_admin = array_merge($permissions_admin, $funcionarioPermissions);

        // ✅ Finalmente, asignas al rol admin
        ...
        // Asignación de PERMISOS para los ROLES de la aplicación SSTSENA (Sincronización de las relaciones sin eliminar las relaciones existentes)

...
class RolesTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $rol_admin = Role::updateOrCreate(['slug' => 'sstsena.admin'], [
           ...
           $rol_admin = Role::updateOrCreate(['slug' => 'sstsena.funcionario'], [
            'name' => 'sstsena funcionario',
            'description' => 'Rol funcionario de la aplicación SSTSENA',
            'description_english' => 'SSTSENA application administrator role',
            'full_access' => 'no',
            'app_id' => $app->id
        ]);
        $useradministrador = User::where('nickname', 'Wchilito')->firstOrFail();
         $useradministrador = User::where('nickname', 'Yeferson28')->firstOrFail();

        $useradministrador->roles()->syncWithoutDetaching([$rol_admin->id]);
         $userfuncionario->roles()->syncWithoutDetaching([$rol_admin->id]);
    }
}

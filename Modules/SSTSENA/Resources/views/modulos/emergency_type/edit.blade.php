...
<div class="container mt-5">
    <div class="card shadow-sm rounded-3" style="background-color: #ffffff;">
        <div class="card-header" style="background-color: #f8f9fa; border-block-end: 1px solid #dee2e6;">
            <h3 class="text-center" style="color: #1a3c6e; font-weight: 600;">Editar Tipo de Emergencia</h3>
        </div>
        <div class="card-body p-4">
            <form action="{{ route('sstsena.admin.emergency_type.update', $emergencyType->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label for="name" class="form-label" style="color: #34495e; font-weight: 500;">Nombre</label>
                    <input type="text" class="form-control border-light-subtle" id="name" name="name" value="{{ $emergencyType->name }}" required>
                </div>
                <div class="mb-4">
                    <label for="description" class="form-label" style="color: #34495e; font-weight: 500;">Descripción</label>
                    <textarea class="form-control border-light-subtle" id="description" name="description" rows="4" required>{{ $emergencyType->description }}</textarea>
                </div>
